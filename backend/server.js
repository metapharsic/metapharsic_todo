...
const fetchAdmin = async () => {
  try {
    const res = await pool.query('SELECT id, name, email, phone, phone_verified, callmebot_apikey, notification_email, notification_whatsapp FROM users WHERE id = 1');
    return res.rows[0];
  } catch (err) {
    return null;
  }
...
app.post('/api/issues', async (req, res) => {
  try {
    ...
    const issueRes = await pool.query(`
      INSERT INTO issues (key, type, title, description, status, priority, assignee_id, reporter_id, story_points, epic_id, sprint_id, due_date, created_at, recurrence, notification, attach_count, department_id)
...
app.put('/api/issues/:key', async (req, res) => {
  try {
    ...
    // Department
    if (patch.department !== undefined && patch.department !== existing.department_id) {
      historyInserts.push({ eventType: 'domain_changed', oldValue: existing.department_id || 'None', newValue: patch.department });
    }
    // Epic
    ...
    if (patch.department !== undefined) { fields.push(`department_id = $${index++}`); values.push(patch.department); }
    ...
    // ── Immediate Notifications for Modifications ────────────────────────────
